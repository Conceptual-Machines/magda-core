...
TEST_CASE("Warp edits without a bridge notify readers and undo the authored map", "[warp][2672]") {
    ...
    undo.executeCommand(std::make_unique<AddWarpMarkerCommand>(nullptr, f.id, 3.0, 4.0));
    ...
    undo.executeCommand(std::make_unique<MoveWarpMarkerCommand>(nullptr, f.id, 1, 5.0));
    undo.executeCommand(std::make_unique<MoveWarpMarkerCommand>(nullptr, f.id, 1, 6.0));
    ...
    undo.executeCommand(std::make_unique<RemoveWarpMarkerCommand>(nullptr, f.id, 1));
    CHECK(f.displayed == std::vector<WarpMarker>{{0, 0}, {8, 8}});
    REQUIRE(undo.undo());
...
TEST_CASE("Native warp edits preserve boundaries and reject degenerate segments", "[warp][2672]") {
    ...
    MoveWarpMarkerCommand crossing(nullptr, f.id, 1, 50.0);
    crossing.execute();
    ...
    for (double source : {0.0, 3.0, 8.0, -1.0, 9.0}) {
        AddWarpMarkerCommand invalid(nullptr, f.id, source, 4.0);
        invalid.execute();
        invalid.undo();
        CHECK(f.event().warpMarkers == before);
    }
    MoveWarpMarkerCommand invalid(nullptr, f.id, 1, std::numeric_limits<double>::quiet_NaN());
    invalid.execute();
    CHECK(f.event().warpMarkers == before);
    RemoveWarpMarkerCommand boundary(nullptr, f.id, 0);
    boundary.execute();
...
TEST_CASE("Warp reposition is one undoable operation without an audio bridge", "[warp][2672]") {
    ...
    auto& undo = UndoManager::getInstance();
    {
        ...
        undo.executeCommand(std::make_unique<RemoveWarpMarkerCommand>(nullptr, f.id, 1));
        undo.executeCommand(std::make_unique<AddWarpMarkerCommand>(nullptr, f.id, 5.0, 6.0));
    }
