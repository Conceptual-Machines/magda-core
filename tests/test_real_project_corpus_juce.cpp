 ...
 *
 * Measured: twenty seconds for the seven, both legs and the comparison, on a
 * debug build. Half of it is the demo project alone -- eight tracks, four
 * instruments, a master limiter and a stretched clip, ten seconds of it -- and
 * the other six are a second or three each. The clip corpus beside it is
 * budgeted at thirty, so this roughly doubles what the null-diff suite costs
 * and it stays in the ordinary job.
 *
