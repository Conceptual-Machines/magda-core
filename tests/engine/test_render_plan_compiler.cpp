...
TEST_CASE("An internal audio route reads the source track's post-mute output",
    ...
    const auto gate = inputOp(plan, destInput, 1);
    REQUIRE(gate != magda::engine::INVALID_OP_ID);
    CHECK(plan.ops[static_cast<std::size_t>(gate)].key.role == OpRole::InputRouteGate);
...
TEST_CASE("Latency compensation goes where paths can arrive apart",
    ...
    SECTION("a sum of two, but not a sum of one") {
        // A track's clips, arrangement and session, are one source (#2301), so an ordinary
        // track is a sum of one and has no delays. The master over two tracks sums two.
        std::vector<TrackInfo> tracks{makeTrack(1), makeTrack(2)};
        const auto plan = magda::engine::compileRenderPlan(tracks, makeMaster());
        requireWellFormed(plan);
        ...
        CHECK(delaysInto(plan, inputs[0]) == 0);  // track 1, over its clips
        CHECK(delaysInto(plan, inputs[1]) == 0);  // track 2, likewise
        CHECK(delaysInto(plan, inputs[2]) == 2);  // the master, over two tracks
