    ...
    void paint(juce::Graphics& g) override {
        ...
        // Draw vertical grid lines in three passes: subdivisions, beats, bars
        {
            ...
            // Pass 4: Labels at grid line positions (bar, beat, subdivision)
            {
                ...
                int subdivsPerBeat = (gridResolutionBeats_ > 0.0)
                                         ? static_cast<int>(std::round(1.0 / gridResolutionBeats_))
                                         : 1;
                ...
                if (pixelsPerBeat_ >= 50) {
                    for (int b = 1; b <= static_cast<int>(beatsVisible) + 1; b++) {
                        if (b % tsNum == 0)
                            continue;
                        int x = static_cast<int>(static_cast<double>(b) * pixelsPerBeat_) +
                                GRID_LEFT_PADDING;
                        if (x > maxX)
                            break;
                        int bar = b / tsNum + 1;
                        ...
                        g.drawText(juce::String(bar) + "." + juce::String(beatInBar), x + 3, labelY,
                                   40, labelHeight, juce::Justification::centredLeft);
                    }
                }

                // Subdivision labels (shown when zoomed in enough)
                if (gridResolutionBeats_ > 0.0 && pixelsPerSubdiv >= 30) {
                    int numLines =
                        static_cast<int>(std::ceil((beatsVisible + 1.0) / gridResolutionBeats_));
                    for (int i = 0; i <= numLines; i++) {
                        double beat = i * gridResolutionBeats_;
