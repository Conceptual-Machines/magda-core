...
std::shared_ptr<FaustPlugin::FaustState>
FaustPlugin::compile(const juce::String& source, int sampleRate, juce::String& errorOut) {
    std::string err;
...
FaustPlugin::FaustPlugin(const te::PluginCreationInfo& info) : te::Plugin(info) {
    ...
    DBG("FaustPlugin ctor: name=" << dspName_
                                  << " in=" << (compiled ? compiled->dspIn : -1)
...
bool FaustPlugin::loadDspSource(const juce::String& name, const juce::String& source,
    ...
    DBG("FaustPlugin::loadDspSource ok name=" << name
                                              << " in=" << compiled->dspIn
                                              << " out=" << compiled->dspOut
                                              << " params=" << static_cast<int>(bindings_.size()));
    return true;
...
void FaustPlugin::initialise(const te::PluginInitialisationInfo& info) {
    ...
    DBG("FaustPlugin::initialise sr=" << currentSampleRate_ << " blockSize="
...
void FaustPlugin::restorePluginStateFromValueTree(const juce::ValueTree& v) {
    ...
    if (savedSource.isNotEmpty() && savedSource != dspSource_) {
        juce::String err;
        if (!loadDspSource(savedName.isNotEmpty() ? savedName : juce::String("Loaded"),
                            savedSource, err)) {
