...
MagdaPluckCompiledPlugin::~MagdaPluckCompiledPlugin() {
...
void MagdaPluckCompiledPlugin::rebuildEngineState(int sampleRate) {
    currentSampleRate_ = sampleRate;
    poly_ = std::make_unique<mydsp_poly>(new MagdaPluckDsp(), kNumVoices, /*control*/ true,
                                         /*group*/ false);
    ...
    for (auto& slot : voiceZonesBySlot_)
        slot.clear();
    for (int i = 0; i < kVoiceSlotCount; ++i) {
        if (auto it = harvester.zonesByIdx.find(i); it != harvester.zonesByIdx.end())
            voiceZonesBySlot_[static_cast<size_t>(i)] = it->second;
    }
}

magda::ParameterInfo MagdaPluckCompiledPlugin::infoForSlot(int slotIndex) const {
    magda::ParameterInfo info;
    const auto& s =
        hostSlotInfo_[static_cast<size_t>(juce::jlimit(0, kHostSlotCount - 1, slotIndex))];
...
float MagdaPluckCompiledPlugin::slotRealValue(int slotIndex) const {
...
void MagdaPluckCompiledPlugin::buildHostParameters() {
    ...
    hostSlotInfo_[kDampingSlot] = {.name = "Damping",
                                   ...
                                   .maxValue = 1.0f,
                                   .defaultValue = 0.7f};
    hostSlotInfo_[kPluckPosSlot] = {.name = "Pluck Pos",
                                    .scale = ParameterScale::Linear,
                                    .minValue = 0.02f,
                                    .maxValue = 0.98f,
                                    .defaultValue = 0.35f};
    hostSlotInfo_[kBrightnessSlot] = {.name = "Brightness",
                                      .scale = ParameterScale::Linear,
                                      .minValue = 0.0f,
                                      .maxValue = 1.0f,
                                      .defaultValue = 0.4f};
    hostSlotInfo_[kDriveSlot] = {.name = "Drive",
                                 ...
                                 .maxValue = 1.0f,
                                 .defaultValue = 0.0f};
    hostSlotInfo_[kTriggerSlot] = {.name = "Trigger",
                                   .scale = ParameterScale::Discrete,
                                   .minValue = 0.0f,
                                   .maxValue = 1.0f,
                                   .defaultValue = 0.0f,
                                   .choices = {"Chord", "Sync"}};
    hostSlotInfo_[kOrderSlot] = {.name = "Order",
                                 .scale = ParameterScale::Discrete,
                                 .minValue = 0.0f,
                                 .maxValue = 3.0f,
                                 .defaultValue = 0.0f,
    ...
    hostSlotInfo_[kShapeSlot] = {.name = "Shape",
                                 ...
                                 .maxValue = 7.0f,
                                 .defaultValue = 1.0f,
                                 .choices = {"Linear", "Ease In", "Ease Out", "Snap", "Spike",
                                             "S-Curve", "Overshoot", "Bounce"}};
    hostSlotInfo_[kCyclesSlot] = {.name = "Cycles",
                                  .scale = ParameterScale::Discrete,
                                  .minValue = 0.0f,
                                  .maxValue = 7.0f,
                                  .defaultValue = 0.0f,
                                  .choices = {"1", "2", "3", "4", "5", "6", "7", "8"}};
    hostSlotInfo_[kStrumLenSlot] = {.name = "Strum Length",
                                    .unit = "ms",
                                    .scale = ParameterScale::Linear,
                                    .minValue = 1.0f,
                                    .maxValue = 400.0f,
                                    .defaultValue = 90.0f,
                                    .scaleAnchor = 40.0f};
    hostSlotInfo_[kSyncIntervalSlot] = {.name = "Sync Interval",
                                        .unit = "ms",
                                        .scale = ParameterScale::Linear,
                                        .minValue = 60.0f,
                                        .maxValue = 2000.0f,
    ...
    for (int i = 0; i < kHostSlotCount; ++i) {
        ...
        const juce::String id = "magda_pluck_" + slot.name.toLowerCase().replace(" ", "_");
...
void MagdaPluckCompiledPlugin::initialise(const te::PluginInitialisationInfo& info) {
...
void MagdaPluckCompiledPlugin::deinitialise() {
    scratchOut_.setSize(0, 0);
    outPtrs_.clear();
}

void MagdaPluckCompiledPlugin::reset() {
    panic();
    if (poly_)
        poly_->instanceClear();
}

void MagdaPluckCompiledPlugin::handleMidi(const te::MidiMessageArray& midi) {
...
void MagdaPluckCompiledPlugin::scheduleStrum() {
    if (held_.empty())
        return;
    std::vector<Held> notes = held_;
    const int order = static_cast<int>(slotRealValue(kOrderSlot));
    if (order == 0)  // Up
    ...
    const float W = slotRealValue(kStrumLenSlot) * 0.001f * static_cast<float>(currentSampleRate_);
    const int cyc = static_cast<int>(slotRealValue(kCyclesSlot)) + 1;  // Discrete index 0..7 → 1..8
...
void MagdaPluckCompiledPlugin::fireDuePlucks() {
    if (!poly_)
        return;
...
void MagdaPluckCompiledPlugin::panic() {
    pending_.clear();
    limEnv_ = 0.0f;
    if (poly_)
        poly_->instanceClear();
...
void MagdaPluckCompiledPlugin::applyToBuffer(const te::PluginRenderContext& fc) {
    if (!poly_ || !fc.destBuffer || fc.bufferNumSamples <= 0)
        return;

    // Rebuild the strum-curve LUT when the Shape slot changes.
    const int shp = static_cast<int>(slotRealValue(kShapeSlot));
    if (shp != lutShape_) {
        buildLut(shp, lut_);
        lutShape_ = shp;
    }
    ...
    for (int slot = 0; slot < kVoiceSlotCount; ++slot) {
    ...
    const bool chordMode = static_cast<int>(slotRealValue(kTriggerSlot)) == 0;
    if (chordMode) {
        if (collectLeft_ >= 0) {
        ...
        if (syncLeft_ <= 0) {
            scheduleStrum();
            const float ms = slotRealValue(kSyncIntervalSlot);
    ...
    const float gain = juce::Decibels::decibelsToGain(slotRealValue(kGainSlot));
    ...
    // plucks fire sample-accurately. mydsp_poly's mix buffers cap at MIX_BUFFER_SIZE.
    int produced = 0;
...
te::AutomatableParameter* MagdaPluckCompiledPlugin::getSlotParameter(int slotIndex) const {
    if (slotIndex < 0 || slotIndex >= kHostSlotCount)
        return nullptr;
    return hostParams_[static_cast<size_t>(slotIndex)].get();
}

const MagdaPluckCompiledPlugin::HostSlotInfo& MagdaPluckCompiledPlugin::getSlotInfo(
    int slotIndex) const {
...
float MagdaPluckCompiledPlugin::displayValueToNativeValue(int slotIndex, float displayValue) const {
    if (slotIndex < 0 || slotIndex >= kHostSlotCount)
        return displayValue;
    return magda::ParameterUtils::realToNormalized(displayValue, infoForSlot(slotIndex));
}

float MagdaPluckCompiledPlugin::nativeValueToDisplayValue(int slotIndex, float nativeValue) const {
    if (slotIndex < 0 || slotIndex >= kHostSlotCount)
        return nativeValue;
