...
void PluginManager::rebuildSidechainLFOCache() {
    ...
    for (const auto& track : tm.getTracks()) {
        ...
        // Also collect from racks on this track (skip racks with external sidechain)
        {
            ...
            if (!hasRackSidechain) {
                // Resolve the per-mod trigger mode through the rack hierarchy
                // so LFOs inside a rack chain that the user set to MIDI/Audio
                // trigger actually receive note-on dispatches — the previous
                // shortcut hard-coded Free for every rack LFO and silently
                // dropped the user's setting.
...
bool PluginManager::trackNeedsSidechainMonitor(TrackId trackId) const {
    ...
    //
    // We must recurse into rack chains: an LFO on a 4OSC sitting inside a
    // rack chain still listens on this track's MIDI bus for retrigger, so
    // missing it here meant the SidechainMonitor was never installed and
    // self-trigger LFOs silently did nothing in that scope.
