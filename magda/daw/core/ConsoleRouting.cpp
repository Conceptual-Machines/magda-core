    ...
    static const std::vector<AgentSurface> surfaces = {
        ...
        {.id = AgentSurfaceId::PianoRoll,
         ...
         .contextProviders = {Provider::ProjectRevision, Provider::ActiveView, Provider::Selection,
                              ...
                              Provider::Conversation},
         .toolAllowlist = {"project.get",     "chordTrack.get",   "chordTrack.ensure",
                           "tracks.list",     "tracks.get",       "clips.list",
        ...
        {.id = AgentSurfaceId::Mixer,
         ...
         .promptFragments =
             {"Ground recommendations in measured mix context when it is available.",
              "Inspect routing endpoints and current sends before changing connections."},
         .contextProviders = {Provider::ProjectRevision, Provider::ActiveView, Provider::Selection,
         ...
         .toolAllowlist = {"project.get", "tracks.list", "tracks.get", "tracks.update",
                           "routing.endpoints.list", "routing.get", "routing.set", "sends.list",
                           "sends.create", "sends.update", "sends.remove", "devices.list",
                           "devices.listParameters", "devices.setParameter", "devices.setBypassed",
                           "selection.get", "transport.get"},
         .runPolicy = {.maxSteps = 8, .maxMutations = 6, .approveMutations = true},
        ...
        {.id = AgentSurfaceId::Automation,
         ...
         .promptFragments =
             {"Resolve the target and inspect its lane before replacing or clearing points.",
