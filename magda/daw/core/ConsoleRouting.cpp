...
RoutingDecision resolveConsoleIntent(ViewMode view, const RoutingContext& ctx,
    ...
    // 0. Preserve deterministic surface overrides while this compatibility
    //    path still dispatches specialist agents. No classifier is needed.
    if (const auto* overrideSurface = agentSurfaceForAlias(ctx.explicitAgentOverride)) {
        ConsoleIntent overrideIntent = ConsoleIntent::Command;
        switch (overrideSurface->id) {
            case AgentSurfaceId::PianoRoll:
                overrideIntent = ConsoleIntent::Music;
                break;
            case AgentSurfaceId::Session:
                overrideIntent = ConsoleIntent::Session;
                break;
            case AgentSurfaceId::Mixer:
            case AgentSurfaceId::Master:
                overrideIntent = ConsoleIntent::Mixing;
                break;
            case AgentSurfaceId::Automation:
                overrideIntent = ConsoleIntent::Automation;
                break;
            case AgentSurfaceId::Drummer:
                overrideIntent = ConsoleIntent::Drum;
                break;
            case AgentSurfaceId::Arrangement:
            case AgentSurfaceId::DevicePanel:
                overrideIntent = ConsoleIntent::Command;
                break;
        }
        return {
            .intent = overrideIntent, .usedRouter = false, .source = "explicit surface override"};
    }
    ...
    if (surface.mode == RoutingMode::Classified && ctx.allowLegacyRouterFallback && classify) {
