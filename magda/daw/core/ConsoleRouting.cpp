...
const std::vector<AgentSurface>& registeredAgentSurfaces() {
    ...
    static const std::vector<AgentSurface> surfaces = {
        {.id = AgentSurfaceId::Arrangement,
         ...
         .toolAllowlist =
             {
                 "project.get",   "project.save",    "project.setTempo", "project.setTimeSignature",
                 "tracks.list",   "tracks.get",      "tracks.create",    "tracks.update",
                 "tracks.delete", "clips.list",      "clips.get",        "clips.createMidi",
                 "clips.delete",  "clips.move",      "clips.resize",     "clips.duplicate",
                 "clips.update",  "clips.transpose", "clips.quantize",   "clips.sliceNotes",
                 "devices.list",  "selection.get",   "selection.set",    "transport.get",
             },
        ...
        {.id = AgentSurfaceId::Mixer,
         ...
         .promptFragments = {"Ground recommendations in measured mix context when it is available.",
                             "Do not claim send changes: routing operations are unavailable."},
         .contextProviders = {Provider::ProjectRevision, Provider::ActiveView, Provider::Selection,
         ...
         .toolAllowlist = {"project.get", "tracks.list", "tracks.get", "tracks.update",
                           "devices.list", "devices.listParameters", "devices.setParameter",
                           "devices.setBypassed", "selection.get", "transport.get"},
         .runPolicy = {.maxSteps = 8, .maxMutations = 6, .approveMutations = true},
        ...
        {.id = AgentSurfaceId::DevicePanel,
         ...
         .promptFragments = {"Operate only on the focused device or its containing rack.",
                             ...
                             "List presets when useful; applying one remains advisory until a "
                             "corresponding Remote API tool exists."},
         .contextProviders = {Provider::ProjectRevision, Provider::ActiveView, Provider::Selection,
         ...
         .toolAllowlist = {"project.get", "tracks.list", "tracks.get", "devices.list",
                           ...
                           "devicePresets.list", "devices.setBypassed", "devices.listParameters",
                           "devices.setParameter", "devices.setParameterConfig",
                           "devices.openEditor", "racks.create", "racks.remove",
                           "racks.setBypassed", "selection.get", "selection.set"},
