...
bool seedSourceMetadataFromCachedDetection(ClipInfo& clip, double projectBPM) {
    ...
    if (fileDuration > 0.0) {
        clip.setSourceDurationSeconds(fileDuration);
        clip.setSourceBpmPreservingDuration(cachedBPM);
    } else {
        clip.setSourceBpmPreservingDuration(cachedBPM);
    }
...
double resolveAudioSourceDurationSeconds(ClipInfo& clip) {
    ...
    if (clip.audioFilePath.isNotEmpty()) {
        if (auto* thumbnail =
            ...
            if (duration > 0.0) {
                clip.setSourceDurationSeconds(duration);
                return duration;
            }
        }
    }

    if (clip.sourceBpm > 0.0 && clip.sourceTotalBeats > 0.0) {
        double duration = clip.sourceTotalBeats * 60.0 / clip.sourceBpm;
        clip.setSourceDurationSeconds(duration);
        return duration;
    }

    double duration = clip.getSourceLength();
    if (duration > 0.0) {
        clip.setSourceDurationSeconds(duration);
        return duration;
    }
...
ClipId ClipManager::createAudioClip(TrackId trackId, double startTime, double length,
    ...
    if (view == ClipView::Arrangement) {
        ...
        // source" until metadata detection populates sourceTotalBeats.
    ...
    // sourceBpm/sourceTotalBeats land on the canonical update path. Cached
...
void ClipManager::setClipLoopEnabled(ClipId clipId, bool enabled, double projectBPM) {
    if (auto* clip = getClip(clipId)) {
        if (clip->type == ClipType::Audio && clip->autoTempo && !enabled) {
            // Beat-mode audio needs TE's loop range active. Disabling the
            // button must not put the model into a half beat-mode state.
            if (!clip->loopEnabled) {
                clip->loopEnabled = true;
                notifyClipPropertyChanged(clipId);
            }
...
void ClipManager::setLengthBeats(ClipId clipId, double newBeats, double bpm) {
    ...
    // sourceTotalBeats) is NOT touched here. Stretch is determined by TE from
...
void ClipManager::applyAudioClipBeats(ClipId clipId, const AudioClipBeatsUpdate& update,
    ...
    // (1) Source facts + interpretation. Inspector BPM / total-beat
    // corrections and BPM-detection callbacks land here. Never touched by
    // clip resize/extend.
    if (update.sourceDurationSeconds)
        clip->setSourceDurationSeconds(*update.sourceDurationSeconds);
    if (update.sourceBpm && update.sourceTotalBeats) {
        clip->sourceBpm = juce::jmax(0.0, *update.sourceBpm);
        clip->sourceTotalBeats = juce::jmax(0.0, *update.sourceTotalBeats);
        if (clip->sourceDurationSeconds <= 0.0 && clip->sourceBpm > 0.0 &&
            clip->sourceTotalBeats > 0.0) {
            clip->sourceDurationSeconds = clip->sourceTotalBeats * 60.0 / clip->sourceBpm;
        }
    } else if (update.sourceBpm) {
        clip->setSourceBpmPreservingDuration(*update.sourceBpm);
    } else if (update.sourceTotalBeats) {
        clip->setSourceTotalBeatsPreservingDuration(*update.sourceTotalBeats);
    }
...
void ClipManager::setAudioSourceBPM(ClipId clipId, double sourceBpm, double projectBPM) {
    auto* clip = getClip(clipId);
    if (!clip || clip->type != ClipType::Audio || sourceBpm <= 0.0)
        return;

    double duration = resolveAudioSourceDurationSeconds(*clip);

    if (clip->autoTempo) {
        AudioClipBeatsUpdate u;
        u.sourceDurationSeconds = duration;
        u.sourceBpm = sourceBpm;
        if (duration > 0.0)
            u.sourceTotalBeats = duration * sourceBpm / 60.0;
        applyAudioClipBeats(clipId, u, projectBPM);
    } else {
        clip->setSourceBpmPreservingDuration(sourceBpm);
        notifyClipPropertyChanged(clipId);
    }
}

void ClipManager::setAudioSourceTotalBeats(ClipId clipId, double sourceTotalBeats,
                                           double projectBPM) {
    auto* clip = getClip(clipId);
    if (!clip || clip->type != ClipType::Audio || sourceTotalBeats <= 0.0)
        return;
    ...
    if (clip->autoTempo) {
        AudioClipBeatsUpdate u;
        u.sourceDurationSeconds = duration;
        u.sourceTotalBeats = sourceTotalBeats;
        if (duration > 0.0)
            u.sourceBpm = sourceTotalBeats * 60.0 / duration;
        applyAudioClipBeats(clipId, u, projectBPM);
    } else {
        clip->setSourceTotalBeatsPreservingDuration(sourceTotalBeats);
        notifyClipPropertyChanged(clipId);
    }
}
    ...
    for (const auto& clipData : clipboard_) {
        ...
        if (newClipId != INVALID_CLIP_ID) {
            ...
            if (newClip) {
                ...
                if (clipData.sourceDurationSeconds > 0.0)
                    newClip->sourceDurationSeconds = clipData.sourceDurationSeconds;
                if (clipData.sourceBpm > 0.0)
                    newClip->sourceBpm = clipData.sourceBpm;
                if (clipData.sourceTotalBeats > 0.0)
                    newClip->sourceTotalBeats = clipData.sourceTotalBeats;
