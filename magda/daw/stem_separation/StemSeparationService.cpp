...
// Decode a whole audio file, all channels. Returns false on failure.
...
void StemSeparationService::splitClipIntoStems(ClipId sourceClipId, Engine engine,
    ...
    pool_->addJob([this, sourceClipId, engine, filePath, baseName, sourceTrackId, startBeat,
        ...
        if (!decodeFile(filePath, input, sampleRate)) {
            failAndCleanup("Could not read audio file");
            return;
        }
        ...
        if (stems.empty()) {
            failAndCleanup("Separation failed");
            return;
        }
        ...
        juce::MessageManager::callAsync([sourceClipId, sourceTrackId, startBeat, sceneIndex, bpm,
                                         stemTrackIds, groupId, stemFiles = std::move(stemFiles),
                                         report, failAndCleanup]() mutable {
            ...
            for (size_t i = 0; i < stemFiles.size() && i < stemTrackIds.size(); ++i) {
                ...
                if (sceneIndex >= 0)
                    dupCmd = magda::DuplicateClipCommand::forSessionSlot(sourceClipId, trackId,
                                                                         sceneIndex);
                else
