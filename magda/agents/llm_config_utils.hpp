...
inline juce::String defaultBaseUrl(llm::Provider provider) {
    switch (provider) {
        case llm::Provider::OpenAIChat:
            return "https://api.openai.com/v1";
        case llm::Provider::OpenAIResponses:
            return "https://api.openai.com/v1";
        case llm::Provider::Anthropic:
            return "https://api.anthropic.com/v1";
        case llm::Provider::Gemini:
...
inline llm::ProviderConfig toLLMProviderConfig(const Config::AgentLLMConfig& config) {
    ...
    pc.baseUrl = config.baseUrl.empty() ? defaultBaseUrl(provider) : juce::String(config.baseUrl);
