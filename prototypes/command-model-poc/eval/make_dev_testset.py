...
CASES = [
    ...
    ("arm the drums for recording", [{"type": "unsupported"}], ["transport"]),
    ("can you undo the last thing", [{"type": "unsupported"}], ["history"]),
    ...
    ("mute the Verse clip on Keys",
     [{"type": "clip_mute", "name": "Keys", "clip_name": "Verse", "enabled": False}], []),
    ("disable clip 2 on Sub",
     [{"type": "clip_mute", "name": "Sub", "index": 2, "enabled": False}], ["colloquial"]),
    ("switch the Intro clip back on",
     [{"type": "clip_mute", "name": "", "clip_name": "Intro", "enabled": True}],
     ["unnamed", "colloquial"]),
    ("silence clip 1 please", [{"type": "clip_mute", "name": "", "index": 1, "enabled": False}],
     ["unnamed"]),
    ("i could do with four tracks",
     [{"type": "create_track", "name": ""} for _ in range(4)], ["filler"]),
    ("chuck in 6 tracks", [{"type": "create_track", "name": ""} for _ in range(6)], []),
    ("knock up two perc tracks",
     [{"type": "create_track", "name": "Perc"} for _ in range(2)], ["colloquial"]),
    ("put a clip at bar 17 on Choir",
     [{"type": "clip_new", "name": "Choir", "length_bars": 4, "bar": 17}], ["colloquial"]),
    ("new 2 bar clip at bar 5",
     [{"type": "clip_new", "name": "", "length_bars": 2, "bar": 5}], ["unnamed"]),
    ("jump to track 4", [{"type": "select_tracks", "id": 4}], ["colloquial"]),
    ("grab track3 and track6",
     [{"type": "select_tracks", "id": 3}, {"type": "select_tracks", "id": 6}], []),
    ("lump track 2 and track 5 together",
     [{"type": "group_tracks", "ids": [2, 5], "name": "Group"}], ["colloquial"]),
    ("run @eq and @compressor side by side",
     [{"type": "create_rack_parallel", "name": "",
       "plugins": [A("eq"), A("compressor")]}], ["unnamed", "colloquial"]),
    ("parallel rack with @ott and @valhalla on Sub",
     [{"type": "create_rack_parallel", "name": "Sub",
       "plugins": [A("ott"), A("valhalla")]}], []),
    ("insert a @saturator",
     [{"type": "add_plugin", "name": "", "plugin": A("saturator")}],
     ["unnamed", "unseen-alias"]),
    ("i want an @eq on this", [{"type": "add_plugin", "name": "", "plugin": A("eq")}],
     ["unnamed", "filler"]),
