...
| `edit` | Project content and lifecycle: save, tempo, time signature, tracks, routing and sends, clips, notes, session clip launch settings, devices, racks, grooves, automation, and the user's selection. Device parameter writes, focused-macro writes, and opening plugin editor windows are edits too. |
